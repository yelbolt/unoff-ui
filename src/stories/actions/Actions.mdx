...
- Use `isBlocked` for paywalled features, with `onBlock` firing instead of `action` while blocked.
- Use `url` + `isLink` to render as an external link styled as a button.
- Use `hasMultipleActions` to append a caret for a button that also opens a menu of related actions.
- Use `isNew` to show a "New"/"Pro" badge, and `warning` to attach a warning callout — both render in the button's status area.
- Use `preview` to show an image preview alongside the "New"/"Pro" badge.
...
- Use `src` to show a thumbnail preview, or `insert` to fill the same slot with a fragment (an icon, initials, a color swatch) when no image exists; both are forwarded to `Thumbnail`.
- The asset slot — and therefore the `actions` overlay — is rendered as soon as either `src` or `insert` is present; pair it with `actions` for contextual hover actions (e.g. duplicate, delete).
...
- The entire card is interactive via `action`; a click or keypress on anything rendered inside `actions` is automatically excluded from it, so those buttons don't need to call `stopPropagation` themselves.
...
- `icon` / `customIcon` sets the icon for `ICON` type; `label` sets the text for `PRIMARY` type.
- `state="LOADING"` shows a spinner on the trigger; `state="DISABLED"` disables it silently.
...
- `alignment` controls where the list anchors relative to the trigger (`BOTTOM_LEFT` · `BOTTOM_RIGHT` · `TOP_LEFT` · `TOP_RIGHT`).
- `containerId` constrains the list overflow to a specific scrollable container.
- `isBlocked` disables the trigger and shows a Pro badge; `isNew` shows a New badge instead.
- `warning` attaches a warning chip to the trigger; `helper` adds an informational tooltip.
...
- When only one active, non-grouped option is present, renders it directly as a `Button` without a dropdown.
