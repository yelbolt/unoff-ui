...
A floating or inline list of menu options with support for groups, separators, submenus, and selection state. Used internally by `Dropdown` and `Menu`.

<DocTabs>
<Tab label="Usage">

- `options` accepts a `DropdownOption` tree; `type` defines the item kind (`OPTION` · `TITLE` · `SEPARATOR` · `GROUP`).
- Each option can carry `label`, `value`, `shortcut`, `action`, `children` (for `GROUP` submenus), and `isActive` / `isBlocked` / `isNew` flags.
- `selected` highlights the currently active value by matching against `value`.
- `direction="LEFT"` / `"RIGHT"` controls which side submenus expand toward (default `RIGHT`).
- `shouldScroll` enables overflow scrolling for long lists constrained by `containerId`.
- `preview` attaches an image tooltip to Pro / New badges for rich option previews.
