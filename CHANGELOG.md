...
- **Bind the right CSS custom properties on inline text link**: add the prefix `styles` to bind the variable invocation with the right properties - a new text styled as link has been added on the story
...
- **`Bar` component**: Increased minimum height to `xlarge` and added padding to its elements; adjusted z-index values for `DraggableWindow` and `Tooltip` to improve layering and visibility
...
- **`Bar` component**: Corrected `flex` property to ensure proper layout

## [1.23.12] - 2026-04-02

### Changed

- **`Bar` component**: Refactored minimum height handling to use `flex` instead of the `min-height` property
...
- **`FormItem` styles**: Enhanced to support adaptive gap handling

## [1.23.9] - 2026-04-01

### Changed

- **`FormItem` layout**: Corrected `min-width` to `min-height` for better responsiveness
- **`DraggableWindow` `clip` prop**: New prop introduced for improved layout control

## [1.23.8] - 2026-03-30

### Changed

- **`ai` icon**: Updated SVG for all four themes (figma, framer, penpot, sketch)
- **`colors` icon**: Updated SVG for all four themes (figma, framer, penpot, sketch)
